        ...
        for cond in input_conditions:
            if cond in terp_map:
                for t_raw, conf in terp_map[cond]:
                    ...
                    if amount > 0:
                        score += amount * conf
                        reason_text = f"✔ {terp.title()} ({amount}) helps with {cond}"
                        reason_text += f" (confidence {conf})" if conf != 1 else " (confidence unknown)"
...
if mode == "Find Strains by Conditions":
    ...
    input_conditions_raw = [x.strip() for x in condition_input.split(",") if x.strip()]
    all_known_conditions = list(terp_map.keys())
    input_conditions = []
    for ic in input_conditions_raw:
        match = difflib.get_close_matches(ic, all_known_conditions, n=1, cutoff=0.6)
        if match:
            input_conditions.append(match[0])
        else:
            input_conditions.append(ic)  # fallback to raw if no match
    ...
    if input_conditions:
        ...
        for name, score, reasons, _ in results[:5]:
            ...
            for r in reasons:
                st.write(r)
            st.markdown("---")
        st.subheader("⚠️ Top 5 Strains to Avoid")
        avoid_list = [(name, reasons) for name, _, reasons, avoid in results if avoid][:5]
        for name, reasons in avoid_list:
